...
vsima se osype z radku delsich nez 1024 znaku, zkompaktnime ORDERS
"""
...
class Preprocessor:
		...
		while idx < len(lines):
			...
			if self.in_new_comment:
				if stripped.lower().startswith('#comm_end'):
					...
					# OPRAVA: Expanze i uvnitř víceřádkového komentáře
					expanded_line = line
					if local_vars:
						expanded_line = self.expand_exprs(line, local_vars)
					self.output.append(expanded_line)
				continue

			if stripped.startswith('#'):
				# Speciální kontrola pro jednorázový komentář v makru (#//)
				if stripped.startswith('#//'):
					continue
				...
				if match:
					...
					rest_of_line = rest_of_line.split("//", 1)[0].rstrip()
					...
					if kw == 'hide':
						...
						self.hide_depth -= 1
						continue
					elif kw == 'none':  # Alternativa k #//
						continue
					elif kw == 'comm_start':
						# OPRAVA: Expanze na řádce s comm_start
			...
			# Expanze výrazů se musí provést DŘÍVE než testování na dolar
			if local_vars:
				line = self.expand_exprs(line, local_vars)
				stripped = line.strip()

			if self.in_order:
				self.order_buffer += re.sub(r',\s+', ',', line).strip()
				if ';' in line:
					self.in_order = False
	...
	def handle_call(self, text, parent_vars):
		...
		new_vars["date"] = datetime.now().strftime("%Y.%m.%d")
		new_vars["time"] = datetime.now().strftime("%H:%M:%S")
