...
class Preprocessor:
    ...
    def process_lines(self, lines, local_vars):
        ...
        while idx < len(lines):
            ...
            # Expanze výrazů se musí provést DŘÍVE než testování na $             if local_vars:
    ...
    def eval_expr(self, expr, local_vars):
        ...
        # Odstranění zbytků závorek a uvozovek
        expr = expr.replace('{', '').replace('}', '').replace("'", "").replace('"', '')
        
        # OPRAVA: Chytré rozlišení proměnných a hexa čísel
        def replace_token(m):
            token = m.group(0)
            # 1. Pokud je to známá proměnná z makra, necháme ji být (Python si s ní poradí)
            if token in local_vars:
                return token
            # 2. Pokud slovo obsahuje jen hexa znaky, přidáme 0x (aby to Python bral jako číslo)
            if re.match(r'^[0-9A-Fa-f]+$', token):
                return "0x" + token
            # 3. Jinak to necháme být
            return token
            
        # Aplikujeme nahrazení na všechna "slova" ve výrazu
